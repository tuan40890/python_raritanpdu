...
def generate_key_and_encrypt_credentials():
    ...
    print("For better security, consider storing the key in an environment variable or a dedicated secret management solution.")

if __name__ == "__main__":
    if os.path.exists(KEY_FILE) or os.path.exists(CREDS_FILE):
        response = input(f"Warning: '{KEY_FILE}' or '{CREDS_FILE}' already exist. Overwrite? (y/N): ")
        if response.lower() != 'y':
