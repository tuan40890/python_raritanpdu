...
def process_pdu(ip, user, pw):
    out = [f"\n--- Processing PDU: {ip} ---"]
    safe = ip.replace('.','_').replace(':','-'); fn = f"pdu_info_{safe}.txt"
    try:
        a = rpc.Agent("https", ip, user, pw, disable_certificate_verification=True)
        net = net_rpc.Net("/net", a); pdu = pdumodel.Pdu("/model/pdu/0", a)
        s, m, ns = pdu.getSettings(), pdu.getMetaData(), net.getSettings()
        eth = ns.ifMap.get("eth0"); dns = ', '.join(ns.common.dns.serverAddrs) if ns.common.dns.serverAddrs else 'N/A'
...
[POWER USAGE INFO & OUTLET DESCRIPTIONS]""")
        ...
        inlet_power = int(inlets[-1].getSensors().activePower.getReading().value) if inlets else 0
        ...
        if rows:
            df = pd.DataFrame(rows, columns=["Watts","Outlet","Status","Name"])
            out.append(df.to_string(index=False, justify="left"))
        else:
            out.append("\tNo outlets found.")
...
if __name__ == "__main__":
    ...
    user, pw = load_creds()
    if not os.path.exists(PDU_FILE): print(f"Missing '{PDU_FILE}'"); sys.exit(1)
    ips = [l.strip() for l in open(PDU_FILE) if l.strip()]
    if not ips: print("No IPs found. Exiting."); sys.exit(0)
